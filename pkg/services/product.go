...
import (
	...
	"github.com/sanprasirt/go-grpc-product-svc/pkg/db"
	"github.com/sanprasirt/go-grpc-product-svc/pkg/models"
	"github.com/sanprasirt/go-grpc-product-svc/pkg/pb"
)
...
func (s *Server) DecreaseStock(ctx context.Context, req *pb.DecreaseStockRequest) (*pb.DecreaseStockResponse, error) {
	...
	// Find the first product id for descrease stock if not nil decrease product stock and then return not found
	if result := s.H.DB.First(&product, req.Id); result.Error != nil {
	...
	if result := s.H.DB.Where(&models.StockDecreaseLog{OrderId: req.OrderId}).First(&log); result.Error == nil {
		return &pb.DecreaseStockResponse{
			Status: http.StatusConflict,
			Error: "Stock already decreaseed",
		}, nil
	}
